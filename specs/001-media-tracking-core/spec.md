...
A user wants to sync their data across devices. They tap "Sign In", enter their email, receive a magic link, and tap it to authenticate. No password required.

**Why this priority**: Sync enables multi-device use. Passwordless reduces friction significantly.

**Independent Test**: User can sign in with only an email address and sync data to cloud.

**Acceptance Scenarios**:

1. **Given** the user is not signed in, **When** they enter their email and tap "Send Magic Link", **Then** they receive an email within 30 seconds
2. **Given** the magic link email arrives, **When** the user taps the link, **Then** they are authenticated and redirected to the app
3. **Given** the user is authenticated on a new device, **When** they open the app, **Then** their existing lists and progress appear
...
- **SC-007**: Magic link authentication completes in under 60 seconds (email to authenticated)
...
- ListenAPI provides podcast episode lists with host/guest metadata
- Supabase free tier is sufficient for initial user base
- Users have email access on the device where they use the app (for magic links)
